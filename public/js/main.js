(function($, window, document) {
	...
	$(function() {
		...
		$('.ability').each(function(index, el) {
			var desc = "";
		...
		$.each($('.str-stat'), function() {
			...
			$.each(stats, function(index, val) {
				var nmod = nmod_calc(nature, val);
				if (nmod == 1.1) {
					$(' <i class="fa fa-arrow-up"></i>').insertAfter(stat_table.find('#base_'+val))
				};
				if (nmod == 0.9) {
					$(' <i class="fa fa-arrow-down"></i>').insertAfter(stat_table.find('#base_'+val))
				};
			});
		});
